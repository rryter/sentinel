...
func NewEnhancedBaseRule(id, name, description string) *EnhancedBaseRule {
	return &EnhancedBaseRule{
		BaseRule: patterns.NewBaseRule(id, name, description),
	}
}

// Match implements the Rule interface
func (r *EnhancedBaseRule) Match(node map[string]interface{}, filePath string) []patterns.Match {
...
func (r *EnhancedBaseRule) ProcessASTNodes(body map[string]interface{}, filePath string, maxDepth int, processor func(node map[string]interface{}) []patterns.Match) []patterns.Match {
	return helpers.ProcessASTNodes(body, filePath, maxDepth, processor)
}
...
func (r *EnhancedBaseRule) GetProgramBody(node map[string]interface{}, filePath string) ([]interface{}, bool) {
	return helpers.GetProgramBody(node, filePath)
}
...
func (r *EnhancedBaseRule) CreateMatch(node map[string]interface{}, filePath string, description string, metadata map[string]interface{}) *patterns.Match {
	return helpers.CreateMatch(r, node, filePath, description, metadata)
}
